 ...
 * You can access them via getAllCategories(). It's doubly linked tree with link
 * to CategoryTree from each category *
 */
public class CategoryTree {
	...
	/**
	 * all categories contained in this category tree. Doesn't matter at what level
	 * they are. They are stored here so that it can be assured that categories have
	 * unique ids along the whole tree.
	 */
	@NotNull
	...
	/**
	 * checks whether category is unique in this tree and if so, then stores the
	 * category, to make sure that in the future new categories have different ids.
	 * Also doesn't let adding new category if maximum number of categories was
	 * reached
	 * 
	 * @param category category that is supposed to be added
	 * @return true, if category is unique and was successfully added, false, if
	 *         category hadn't got unique id or maximum number of categoireies is
	 *         reached.
	 */
	 ...
	 * @return category with categoryId or null if there is not such category
	 */
	@Nullable
	...
	public int getUniqueId() {
		...
		boolean found = false;
		int proposedId = -1;
		while (!found) // maximum number of categories is set, so it won't overflow and always will
						// find a number
		{
			proposedId++;
