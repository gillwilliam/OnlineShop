...
<body>
	...
	<%
		...
		if (useOtherUser) {
			...
			otherBuyer.setLastName(request.getParameter(application.getInitParameter("surname")));
	...
	<div id="container">
		...
		<div id="message_box"
			class="<%=validationResult == null && updateResult == null ? "invisible" : ""%>"
		...
		<form id="user_data_form"
			action="${pageContext.request.contextPath}/editBuyerProfile.main"
			method="post">
			...
			<section id="personal_data_section">
				...
				<input id="name" type="text"
					...
					value="<%=useOtherUser ? otherBuyer.getFirstName() : user.getFirstName()%>" />

				<span class="error"><%=nameMessage%></span> <input id="surname"
					type="text" name="<%=application.getInitParameter("surname")%>"
					placeholder="Surname"
					value="<%=useOtherUser ? otherBuyer.getLastName() : user.getLastName()%>" />
				...
				<span class="error"><%=phoneMessage%></span> <input id="address"
        ...
        ${pageContext.request.contextPath}/deleteBuyer<%= application.getInitParameter("main_front_controller_request_extension") %>">
			...
			<input type="hidden"
				...
				name="requester" value="buyer" /> <input id="but_delete_account"
